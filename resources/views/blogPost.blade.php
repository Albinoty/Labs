                    ...
                    <div class="single-post">
                                ...
                                @if ($user->id == $article->id_user)
                                    <div class="avatar w-25 ">
                                        @if ($user->img_user == null || $user->img_user == "img/avatar/john-doe.png")
                                            <img src="{{asset('/img/avatar/john-doe.png')}}" class="img-fluid" alt="">
                                        @else
                                            <img src="{{asset('/storage/'.$user->img_user)}}" class="img-fluid" alt="">
                                        @endif
                                    </div>
                        ...
                        <div class="comments" id="comments">
                            ...
                            <ul class="comment-list">
                                @forelse ($commentaires as $commentaire)
                                    <li>
                                        <div class="avatar">
                                            @if ($commentaire->img_user == "img/avatar/john-doe.png")
                                                <img src="{{asset('/img/avatar/john-doe.png')}}" alt="">
                                            @elseif (substr($commentaire->img_user,0,3) == "img")
                                                <img src="{{asset('/storage/'.$commentaire->img_user)}}" alt="">
                                            @else
                                                <img src="{{asset($commentaire->img_user)}}" alt="">
                                            @endif
                                        </div>
                                            ...
                                            <h3 class="d-flex align-items-center">{{$commentaire->nom}} | 
                                                ...
                                                @if(Auth()->user()!= null)
                                                    @if (Auth()->user()->email == $commentaire->email || Auth()->user()->role == "admin")
                                                        <form action="/post/{{$commentaire->id}}/commentaire" method="POST">
                                                            @csrf
                                                            @method('delete')
                        ...
                        <div class="row">
                            <div class="col-md-9 comment-from">
                                ...
                                <form class="form-class" action="/post/{{$article->id}}/commentaire" method="POST">
                                    @csrf
                                    @method('post')
                                    <div class="row mt-2">
                                        @if(Auth::user() == null)
